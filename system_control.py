...
def control_windows():
    layout = [
            ...
            [sg.Text('Volume/flush [mL]', size=(15, 1), font='Helvetica 12'), sg.InputText('1')],
...
def main():
    ...
    while True:
        ...
        if event in ('Start', None):
            flow_rate, volume, step_volume = int(values[0]), int(values[1]), int(values[2])


            e_cond, t_cond = float(values[3]), float(values[4])
            e_dep = float(values[5])
