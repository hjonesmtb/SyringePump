...
def conduct_measurements(pstat, pump, window):
    ...
    window['-TEST_STATUS-'].update('Initial flow-through phase. When system is ready (~60s), hit load sample while turning sample loop.')
...
def stop_flow(pump, pstat, valve_turned):
    print('valve turned', valve_turned)
    if not valve_turned:
        pump.infuse()
